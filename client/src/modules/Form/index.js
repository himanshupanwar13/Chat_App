...
const Form = ({
  ...
  return (
    <div className="bg-light h-screen flex justify-center items-center">
      ...
      <form onSubmit={() => console.log("Submitted")} className="flex flex-col w-1/2 justify-center" >
        {!isSignInPage && <Input label="Full name" name="name" placeholder="Enter your FullName" className="w-full mb-6" value={data.fullName} onChange={(e)=> setData({ ...data, fullName : e.target.value})} />}
